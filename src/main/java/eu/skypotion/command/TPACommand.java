...
public class TPACommand extends Command {
    ...
    public TPACommand(PotionPlugin plugin) {
        super("tpa", "", "", List.of("tprequest", "tpanfrage"));
        this.plugin = plugin;
    }
    ...
    public boolean execute(CommandSender commandSender, String s, String[] args) {
        if (commandSender instanceof Player player) {
            ...
            if (args.length == 0) {
                player.sendMessage(ProjectConstants.INVALID_USAGE.formatted("/tpa <Spieler>"));
                return false;
            }
            ...
            plugin.getTeleportRequestManager().requestTeleport(player.getUniqueId(), target.getUniqueId());
            player.sendMessage(ProjectConstants.PREFIX + "§7Teleportanfrage an §a" + target.getName() + " §7gesendet.");
            target.sendMessage(ProjectConstants.PREFIX + "§a" + player.getName() + " §7möchte sich zu dir teleportieren.");
